...
async function calculateAnalyticsData(filters) {
  ...
  // 3. Filter Expenses (General + Chemical Purchases)
  ...
  // Total Expenses formula: General expenses (monthly averaged) + Chemical purchases
  ...
  const totalExpenses = totalGeneralExpenses + totalChemicalExpenses;
  ...
  // Expense Categories
  const expenseCatMap = {
    'Chemical Supplies': totalChemicalExpenses
  };
  filteredGeneralExpenses.forEach(e => {
    const cat = e.expense_name || 'General Operations';
    expenseCatMap[cat] = (expenseCatMap[cat] || 0) + (parseFloat(e.monthly_averaged_amount || e.amount) || 0);
  });

  // Time Bucket Grouping (Daily, Weekly, Monthly, Yearly)
  const timeBuckets = groupDataByTimeGrain(filteredOrders, filteredGeneralExpenses, filteredChemicalPurchases, filters.grain, start, end);
...
// ─────────────────────────────────────────────
function groupDataByTimeGrain(orders, genExpenses, chemPurchases, grain, start, end) {
  const buckets = {};
...
function renderKPICards(d) {
  ...
  container.innerHTML = `
    ...
    ${anStatCard("Operating Expenses", formatCurrency(d.totalExpenses), "fa-receipt", "#ef4444", "#fee2e2", `Cost ratio: ${d.costRatio.toFixed(1)}%`)}
